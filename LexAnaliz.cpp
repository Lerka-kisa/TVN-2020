...
int DecimicalNotation(std::string input, int scaleofnot) {
	std::string lich = input.substr(1, input.size());
	return std::stoi(lich, nullptr, scaleofnot);
}
namespace Lexer
{
	Graph graphs[N_GRAPHS] =
	{
		...
		{ LEX_ID_TYPE, FST::FST(GRAPH_LICHBA) },
	...
	IT::IDDATATYPE getType(char* curword, char* idtype)
	{
		...
		if (!strcmp(TYPE_LICHBA, idtype))
			return IT::IDDATATYPE::LICH;	 // числовой  ид
		if (isdigit(*curword) || *curword == LEX_MINUS)
			return IT::IDDATATYPE::LICH;				// числовой литерал
		...
		bool& lex_ok)							// флаг ошибки(по ссылке)
	{
		...
		else // идентификатор
		{
			...
			case IT::IDDATATYPE::STR:
				...
				break;
			case IT::IDDATATYPE::LICH:
				itentry->value.vint = LICH_DEFAULT;
				break;
			}
			...
			for (int j = 0; j < N_GRAPHS; j++)
			{
				...
				if (FST::execute(fst))
				{
					...
					case LEX_LITERAL_NUMO:
					{
						...
						if (value < LICH_MINSIZE || value > LICH_MAXSIZE)
