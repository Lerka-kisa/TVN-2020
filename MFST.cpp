...
namespace MFST
{
	...
	bool Mfst::savediagnois(RC_STEP prc_step)
	{
		bool rc = false;
		short k = 0;
		while (k < MFST_DIAGN_LICHBA && lenta_position <= diagnosis[k].lenta_position)k++;
		if (rc = (k < MFST_DIAGN_LICHBA))
		{
			diagnosis[k] = MfstDiagnosis(lenta_position, prc_step, nrule, nrulechain);
			for (short j = k + 1; j < MFST_DIAGN_LICHBA; j++)diagnosis[j].lenta_position = -1;
		};
		return rc;
	};
	...
	char* Mfst::getDiagnosis(short n, char* buf)//вывод ошибок
	{
		...
		int errid = 0;
		int lpos = -1;
		if (n < MFST_DIAGN_LICHBA && (lpos = diagnosis[n].lenta_position) >= 0)
		{
